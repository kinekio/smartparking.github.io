...
$booking = mysqli_query($conn,"UPDATE mensh SET status ='Booked' WHERE spot_id=".$postcodes[$x]."");
 if(!$conn){  
  echo "<script type='text/javascript'>alert('Database failed');</script>";
    die('Could not connect: '.mysqli_connect_error());  
}
...
$bookin = mysqli_query($conn,"UPDATE Customer SET location='MENS HOSTEL' WHERE email='$u';");
 if(!$conn){  
  echo "<script type='text/javascript'>alert('Database failed');</script>";
    die('Could not connect: '.mysqli_connect_error());  
}
