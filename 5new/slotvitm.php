...
include ('header2.php');
echo "<div id='buttons'>";
$result = mysqli_query($conn,"SELECT spot_id FROM vit_m WHERE 1");
$postcodes = array();
while ($row = mysqli_fetch_array($result)) {
    $postcodes[] = $row[0];
}
$statuscodes = array();
$result1 = mysqli_query($conn,"SELECT status FROM vit_m WHERE 1");
...
$booking = mysqli_query($conn,"UPDATE vit_m SET status ='Booked' WHERE spot_id=".$postcodes[$x]."");
 if(!$conn){  
  echo "<script type='text/javascript'>alert('Database failed');</script>";
    die('Could not connect: '.mysqli_connect_error());  
}
...
$bookin = mysqli_query($conn,"UPDATE Customer SET location='VIT MAIN GATE' WHERE email='$u';");
 if(!$conn){  
  echo "<script type='text/javascript'>alert('Database failed');</script>";
    die('Could not connect: '.mysqli_connect_error());  
}
